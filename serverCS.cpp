...
serverCS is a department databse server, which replies to and fulfills query 
...
class serverCS
{
    // Private instance variables and functions for the CS department server object
    private:
        map<string, Record*> cs_map;
        int sockfd;
...
/*
	Constructs a serverCS object. Loads database using data from "cs.txt" upon initialization.
*/
serverCS::serverCS(/* args */)
{
    ifstream file;
...
/*
	Void destructor for the serverCS object
*/
serverCS::~serverCS()
{
}
...
*/
string serverCS::int_to_str(int val)
{
	ostringstream oss;
    oss << val;
...
*/
int serverCS::str_to_int(string str_val)
{
...
/*
	...
	@param		word2			pointer to where the 2nd string from
								the split string is stored
*/
void serverCS::split_str(string sentence, string *word1, string * word2)
{
...
*/
void serverCS::split_str_arr(string sentence, int count, string* str_arr)
{
	//string first_half, second_half;
	stringstream line_reader(sentence);
...
/*
	Loads data from the text file "cs.txt" into the database.
...
*/
void serverCS::loadDatabase(ifstream *fp, string fileName)
{   
    ...
    while(getline((*fp), line)) 
    {
        ...
        cs_map[new_record->course_code] = new_record;
    }
...
*/
void serverCS::print_map(map<string, Record*> *map_var)
{
...
*/
void *serverCS::get_in_addr(struct sockaddr *sa)
{
...
*/
int serverCS::send_udp_info(string str_msg, struct sockaddr_storage *their_addr, socklen_t addr_len)
{
    int numbytes;
...
*/
string serverCS::recv_udp_info(sockaddr_storage *their_addr, socklen_t *addr_len)
{
...
/*
	...
	@return		returns 0 if successful or a negative int if unsuccessful
*/
int serverCS::create_udp_server_socket(void)
{
    ...
    // This gets the remote address of the serverCS server
    ...
    cout << "The serverCS is up and running using UDP on port " << CS_UDP_PORT << "." << endl;
...
*/
string serverCS::query_helper(string msg)
{
    string outgoing_msg = "";
    size_t found = msg.find(",");

    // didn't find a "," in the string which means this is a single request 
    // of format --> CS__ (so send all categories)
    if (found == std::string::npos) 
    {
        string course_code = msg;

        cout << "The serverCS recieved received a request from the Main Server ";
        cout << "about " << course_code << "." << endl;

        Record* found_record;
        if (cs_map.find(course_code) != cs_map.end())
        {
            found_record = cs_map[course_code];

            // 1,1,CS450: 4, Ali Zahid, Tue;Thu, Introduction to Computer Networks
            // CS669: 4, Jay Kuo, Mon;Wed, Multimedia Data Compression
...
*/
int serverCS::query_driver(void)
{
    while(TRUE)
    {
        string client_msg;
...
{
    serverCS cs_server;
    cs_server.create_udp_server_socket();
    cs_server.query_driver();
    return 0;
}